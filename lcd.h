...
#define LCD_DATA	PORTA	//PORTA 200429
#define LCD_INST	PORTA	//PORTA
#define LCD_PIN 	PINA	//PINA
#define LCD_CTRL	PORTG	//PORTG	//졸작키트에서는 G번 포트 못쓰니까 
...
void LCDInit(void)
{
	...
	_delay_ms(2);
	writeCTRL(1<<LCD_ON_CTRL | 1<<LCD_ON_DISPLAY | 1<<LCD_ON_CURSOR );	
	_delay_ms(2);		   
	clearLCD();		
	_delay_ms(2);
	writeCTRL(1<<LCD_ENTRY_MODE | 1<<LCD_ENTRY_INC); 
